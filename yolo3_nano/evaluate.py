...
def decode_output(netout,input_shape,image_shape,anchors,conf_thres):
    ...
    for i in range(grid_h*grid_w):
        ...
        for b in range(nb_box):
            ...
            box = [(x-w/2)*image_w, (y-h/2)*image_h, x2, y2, objectness]
            box.extend(classes)
...
def do_nmx_tf(box,iou_thres,num_classes,score_thes):
    ...
    for c in range(num_classes):
        ...
        class_boxes = boxes
        class_box_scores = box_scores[...,0:1] * box_scores[...,1+c:1+c+1]
        class_box_scores = np.concatenate(class_box_scores, axis=0)
        
        mask = class_box_scores > score_thes
        class_boxes = class_boxes[mask]
        class_box_scores = class_box_scores[mask]
        indices  = apply_non_max_suppression(
            class_boxes, class_box_scores, iou_thres,20)
        
        
        class_boxes = class_boxes[indices]
        class_box_scores = class_box_scores[indices]

        boxes = np.ones(shape=(class_boxes.shape[0],6))
        boxes[...,0:4] = class_boxes
        boxes[...,4:5] = class_box_scores.reshape(-1,1)
        boxes[...,5:] = np.ones_like(class_box_scores.reshape(-1,1), 'int32') * c
        boxes_.append(boxes)
...
def get_yolo_boxes(model,anchors,img_data,img_shape,conf_thres,iou_thres,num_classes):
    ...
    if len(pred_boxes) > 0:
        pred_boxes= do_nmx_tf(np.array(pred_boxes),iou_thres,num_classes,conf_thres)
...
def evaluate(model,input_shape,annotation_lines,anchors,num_classes,score_threshold,iou_threshold,class_names,map_out_path):
    ...
    for i in range(n):
        ...
        pred_boxes= get_yolo_boxes(model,anchors,image,input_shape,score_threshold,iou_threshold,num_classes)
        f = open(os.path.join(map_out_path,"detection-results/"+image_id),'w')
        if(len(pred_boxes) > 0):
            for i in range(len(pred_boxes)):
                f.write("%s %f %f %f %f %f\n"%(class_names[int(pred_boxes[i][5])],pred_boxes[i][4],pred_boxes[i][0],pred_boxes[i][1],pred_boxes[i][2],pred_boxes[i][3]))
        else:
