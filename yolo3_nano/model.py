...
from tensorflow.keras.layers import UpSampling2D # type: ignore
...
def mbv1_conv_block(inputs, filters, alpha, kernel=(3, 3), strides=(1, 1)):
    ...
    x = tf.keras.layers.Conv2D(filters,kernel,padding='same',use_bias=False,
...
def mbv1_depthwise_conv_block(inputs,pointwise_conv_filters,alpha,
    ...
    if strides == (1, 1):
        ...
        x = inputs
        x = tf.keras.layers.ZeroPadding2D(((0, 1), (0, 1)), name='conv_pad_%d' % block_id)(inputs)
    x = tf.keras.layers.DepthwiseConv2D((3, 3),padding='same' if strides == (1, 1) else 'valid',
                                depth_multiplier=depth_multiplier,strides=strides,use_bias=False,
                                name='conv_dw_%d' % block_id)(x)
    x = tf.keras.layers.BatchNormalization(axis=channel_axis, name='conv_dw_%d_bn' % block_id)(x)
    x = tf.keras.layers.ReLU(6., name='conv_dw_%d_relu' % block_id)(x)

    x = tf.keras.layers.Conv2D(pointwise_conv_filters, (1, 1),padding='same',
                                use_bias=False,strides=(1, 1),name='conv_pw_%d' % block_id)(x)
    x = tf.keras.layers.BatchNormalization(axis=channel_axis, name='conv_pw_%d_bn' % block_id)(x)
...
def tiny_yolo_res_body(shape, num_anchors, num_classes,alpha=1.0, weights_path = './' ,weight_load=False):
    ...
    x1 = body.output    
    x2 = mbv1_depthwise_conv_block(x1,first_block_filters,1,1,block_id=255)#//16
    x2 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding = 'valid')(x2)
    x3 = mbv1_depthwise_conv_block(x2, first_block_filters, 1,1,block_id=253)#//32
    x3 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding = 'valid')(x3)
    
    x32 = UpSampling2D(2)(x3)#//16
    x2 = tf.keras.layers.add([x2, x32])#32 + 16 
    x32 = mbv1_depthwise_conv_block(x2, first_block_filters, 1,1,block_id=252)
    x2 = tf.keras.layers.add([x2, x32])#32 + 16 
    ...
    x1 = tf.keras.layers.add([x1, x21])#8
    x1 = mbv1_depthwise_conv_block(x1, first_block_filters, 1,1,block_id=250)
    ...
    # //16 branch
    x21 = _conv_block(x21, first_block_filters, (1, 1), strides=(1, 1))#16
    x2 = tf.keras.layers.add([x2, x21])#16 
    x21 = mbv1_depthwise_conv_block(x2, first_block_filters, 1,1,block_id=249)
    x2 = tf.keras.layers.add([x2, x21])#16    
    x2= mbv1_depthwise_conv_block(x2, first_block_filters, 1,1,block_id=248)
    ...
    x32 = mbv1_depthwise_conv_block(x3, 32, 1,1,block_id=247)
    x3 = tf.keras.layers.add([x3, x32])#32
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False, obj_scale=1,noobj_scale=0.5):
...
if __name__ == '__main__':
    ...
    def representative_random_data_gen():
        for b in range(100):
            dat = np.random.randint(low=0,high=255,size=(1,cfg.width,cfg.height,c))
            dat = (dat/256).astype('float32')
            yield [dat]
    ...
    converter.representative_dataset = representative_random_data_gen
    tflite_model_quant = converter.convert() 
    m_path = 'demo_%d_%d_%d.tflite'%(w,h,c)
